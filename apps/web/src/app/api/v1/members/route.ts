...
export async function POST(request: Request) {
  ...
  const isForeignMember = body.isForeignMember === true;
  const nationalId =
    !isForeignMember && typeof body.nationalId === 'string' ? body.nationalId.trim() : null;
  const nationality =
    isForeignMember && typeof body.nationality === 'string'
      ? body.nationality.trim().toUpperCase()
      : null;
  const passportNumber =
    isForeignMember && typeof body.passportNumber === 'string'
  ...
  if (isForeignMember && (!nationality || !passportNumber)) {
    return apiError('Yabancı üye için ülke ve pasaport numarası zorunludur.', 400);
  }

  if (!isForeignMember && nationalId && !/^\d{11}$/.test(nationalId)) {
    return apiError('TC Kimlik No 11 haneli olmalıdır.', 400);
  }
