...
1. Resepsiyon bilgisayarına `SGMS-Resepsiyon-0.4.0-Setup.exe` kopyalanır.
2. Kurulum sihirbazı açılır → CiCiByte SGMS logosu, lisans metni.
3. Kurulum dizini: **`C:\Program Files\Cicibyte\SGMS`** (sabit, değiştirilemez).
4. **Masaüstü kısayolu** ve **Başlat menüsü** otomatik oluşturulur.
5. Kurulum bitince uygulama bir kez açılır → **Resepsiyon Kontrol Merkezi** (markalı giriş ekranı).

**Cuma 17:15 — İlk oturum (Ayşe, resepsiyon)**

1. Ayşe personel hesabıyla giriş yapar — **yalnızca e-posta ve parola**.
2. **Giriş yap ve dinlemeyi başlat** → bağlantı yeşile döner.
...
```text
apps/reception/release/SGMS-Resepsiyon-0.4.0-Setup.exe   # yerel build
releases/sgms-reception/v0.4.0/SGMS-Resepsiyon-0.4.0-Setup.exe   # GitHub dağıtımı
```
...
1. `SGMS-Resepsiyon-0.4.0-Setup.exe` dosyasını çalıştırın.
...
Sunucu adresi ve Soketi anahtarı uygulama tarafından otomatik alınır (`https://sgms.cicibyte.com`).
...
| Bağlantı kapalı | İnternet / firewall; sunucu bakımda olabilir — destek ile iletişime geçin |
